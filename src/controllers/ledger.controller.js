...
import { round3, packingDetailFor, packingTotal, parsePackingRows, stockSearchFilter, txLogSearchFilter } from "../lib/ledgerUtils.js";
...
export async function getLedger(req, res) {
  const [items, locations, txLog] = await Promise.all([
    ...
    TxLog.find(txLogSearchFilter(req.query.logSearch)).sort({ _id: -1 }),
  ]);
